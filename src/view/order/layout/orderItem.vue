<template>
	<div class="OrderItem">
      ...
      <div slot="header" class="vux-1px-b card-header">
        ...
        <p tappable style="font-weight:500;" @click="$emit('on-orderDetail',item)">中通订单号 :  <span class="ticket-code">{{item.order_code}}</span></p>
      </div>
      <div slot="content" class="card-body">
        ...
        <div class="p-h-xs vux-1px-t">
          ...
          <p>原订单号: <span class="text-muted">{{ item.order_code_cust }}</span></p>
  ...
  export default {
    ...
    computed: {
      item(){
        ...
        const target = JSON.parse(JSON.stringify( this.order ))
        const maxLen = 3
        const styleMapping = {
          "待确认": "text-warning",
          "待拣取": "text-warning",
