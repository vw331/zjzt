<template>
	<div id="orderTop">
		<x-header title="订单查询" style="width:100%;position:fixed;left:0;top:0;z-index:100;"
	              :left-options="{showBack: showBack, backText: ''}" :right-options="{showMore: false}">
        <div class="overwrite-title-tab" slot="overwrite-title">
          <x-button mini>订单</x-button><i></i>
          <x-button mini type="primary" :link="{path: '/billSearch'}">发货单</x-button>
        </div>
    ...
    <div v-transfer-dom>
      <popup  v-model="showPopup" position="right" @on-hide="hideRightPopup" width="100%">
        ...
        <group  label-width="6em" label-margin-right="2em" label-align="left">
          <x-input v-model="params.order_code" title="订单号" placeholder="输入订单号"  placeholder-align="right" text-align="right"></x-input>
...
<script>
  import {XHeader, PopupHeader , ButtonTab, ButtonTabItem, TransferDom, Popup, Group, XInput, Datetime, PopupPicker, XButton} from 'vux'
...
<style >
  ...
  #orderTop .menu:before {
    right: -70px;
  }
  #orderTop .overwrite-title {
    margin-top: 5px;
  }
  .vux-datetime p {
    font-weight:bold;
